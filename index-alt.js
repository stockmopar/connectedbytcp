/*
DESCRIPTION:
-This file is an alternate version of stockmopar's index.js file,  which has additional functions
-This File is still a work in progress
-Purpose: Add additional controls
-Additions (so far): almost all are incomplete
  - GetDeviceState
...
var GetStateString = [
  "<gwrcmds><gwrcmd><gcmd>RoomGetCarousel</gcmd><gdata><gip><version>1</version><token>%s</token><fields>name,control,power,product,class,realtype,status</fields></gip></gdata></gwrcmd></gwrcmds>"
].join("\n");

var RoomSendCommand = [
  "<gip><version>1</version><token>%s</token><rid>%s</rid><value>%s</value></gip>"
].join("\n");
var RoomSendLevelCommand = [
  "<gip><version>1</version><token>%s</token><rid>%s</rid><value>%s</value><type>level</type></gip>"
].join("\n");

var DeviceSendCommand = [
  "<gip><version>1</version><token>%s</token><did>%s</did><value>%s</value></gip>"
].join("\n");
var DeviceSendLevelCommand = [
  "<gip><version>1</version><token>%s</token><did>%s</did><value>%s</value><type>level</type></gip>"
].join("\n");

var LogInCommand = [
  "<gip><version>1</version><email>%s</email><password>%s</password></gip>"
].join("\n");
...
TCPConnected.prototype.SyncGateway = function(cb) {
  ...
  var payload = util.format(
    RequestString,
    "GWRLogin",
    encodeURIComponent(gLogInCommand)
  );
  var options = {
    hostname: this._host,
    port: 443,
  ...
  tcpSocket = https.request(options, function(res) {
    res.on("data", function(data) {
      ...
      if (data == "<gip><version>1</version><rc>404</rc></gip>") {
        console.log(
          "Permission Denied: Gateway Not In Sync Mode. Press Button on Gateway to Sync."
        );
        cb(1);
      } else {
...
TCPConnected.prototype.LoadToken = function(cb) {
  ...
  if (nconf.get("token") != undefined) {
    ...
    console.log(
      "No Token Saved. Attempting to Connect With Gateway to Get Token."
    );
...
TCPConnected.prototype.GetDeviceState = function(did, cb) {
  Rooms.forEach(function(room) {
    var devices = room.device;
    if (typeof devices.did !== "undefined") {
      if (devices.did == did) {
        cb(null, devices.state, devices.level);
      }
    } else {
      devices.forEach(function(device) {
        if (device.did == did) {
          cb(null, device.state, device.level);
        }
      });
    }
  });
};
TCPConnected.prototype.GetDeviceStateByName = function(name, cb) {
  Rooms.forEach(function(room) {
    var devices = room.device;
    if (typeof devices.did !== "undefined") {
...
TCPConnected.prototype.GetDIDByName = function(name) {
  var did = 0;
  Rooms.forEach(function(room) {
    var devices = room.device;
    if (typeof devices.did !== "undefined") {
...
TCPConnected.prototype.TurnOnDevice = function(did, cb) {
  ...
  var payload = util.format(
    RequestString,
    "DeviceSendCommand",
    encodeURIComponent(DeviceCommand)
  );
...
TCPConnected.prototype.TurnOnDeviceByName = function(name, cb) {
  var did = this.GetDIDByName(name);

  this.TurnOnDevice(did, cb);
};
...
TCPConnected.prototype.TurnOffDeviceByName = function(name, cb) {
  var did = this.GetDIDByName(name);

  this.TurnOffDevice(did, cb);
};
TCPConnected.prototype.SetDeviceLevel = function(did, level, cb) {
  var DeviceLevelCommand = util.format(
...
TCPConnected.prototype.SetDeviceLevelByName = function(name, level, cb) {
  var did = this.GetDIDByName(name);

  this.SetDeviceLevel(did, level, cb);
};
TCPConnected.prototype.TurnOnDeviceWithLevel = function(did, level, cb) {
  var self = this;

  this.SetDeviceLevel(did, level, function(error) {
    self.TurnOnDevice(did, cb);
  });
};
TCPConnected.prototype.TurnOnDeviceWithLevelByName = function(name, level, cb) {
  var did = this.GetDIDByName(name);
...
TCPConnected.prototype.GetRoomState = function(rid, cb) {
  Rooms.forEach(function(room) {
    if (room["rid"] == rid) {
      var state = 0;
      var i = 0;
...
TCPConnected.prototype.TurnOnRoom = function(rid, cb) {
  ...
  var payload = util.format(
    RequestString,
    "RoomSendCommand",
    encodeURIComponent(RoomCommand)
  );
...
TCPConnected.prototype.TurnOnRoomWithLevel = function(rid, level, cb) {
  var self = this;
...
TCPConnected.prototype.SetRoomLevel = function(rid, level, cb) {
  var RoomLevelCommand = util.format(
    RoomSendLevelCommand,
    this._token,
    rid,
    level
  );
  var payload = util.format(
    RequestString,
    "RoomSendCommand",
    encodeURIComponent(RoomLevelCommand)
  );
...
TCPConnected.prototype.ReturnRoomsArray = function(cb) {
  cb(null, Rooms);
};
